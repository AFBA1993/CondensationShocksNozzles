...
def Gyarmathy_growth(P3,T3,u3,A2,grid,xcond,f,EoS,k):
    ...
    for i in range(ne):
        ...
        xii[i+1]=xif[i]
        xif[i+1]=xii[i+1]+stp
        Ni[i+1]=Ji[i+1]*(xif[i+1]-xii[i+1])/(rhoi[i+1]*ui[i+1])/100
...
Tsat=T_sat(1e+05,f,EoS)
propl=Cool_Tsat(Tsat,f,EoS)
propsat_p=Cool_Psat(1e+05,f,EoS)
"""
